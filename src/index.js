...
const install = (VueOrApp = null, options = {}) => {
  ...
  ;['directive', 'attributes', 'cache', 'intersectionObserverOptions'].forEach(
    (option) =>
      (options[option] = Object.assign({}, DEFAULT_OPTIONS[option], options[option] || {})),
  )
  ...
  for (const option in options.directive) {
    ...
    if (
      ...
      (option === 'name' && !REGEXP_ATTRIBUTE_NAME.test(options.directive[option]))
    )
      throw new TypeError(
        `[${PACKAGE_NAME}] Option is not valid! [options.directives.${option}="${options.directives[option]}"]`,
      )
  }
  ...
  for (const option in options.attributes) {
    ...
    if (!Array.isArray(options.attributes[option]))
      throw new TypeError(
        `[${PACKAGE_NAME}] Option is not valid! [options.attributes.${option}=${JSON.stringify(options.attributes[option])}]`,
      )

    /* cast option values to strings */
    options.attributes[option] =
      option === 'add'
        ? options.attributes[option].map((attribute) => ({
  ...
  for (const option in options.cache) {
    ...
    options.cache[option] =
      option === 'version'
        ? options.cache[option].toString().trim().toLowerCase()
        : !!options.cache[option]
  }
   ...
   */
  const validateAxiosGetMethod = (axios = null) =>
    !!axios && typeof axios === _fnc && 'get' in axios && typeof axios.get === _fnc
  ...
  options.axios =
    ((axiosIsValid = validateAxiosGetMethod(options.axios)) ? options.axios : null) ||
    (options._axios && 'create' in axios && typeof axios.create === _fnc ? axios.create() : null) // eslint-disable-line no-cond-assign
  ...
  if (!options._observer)
    console.error(
      `[${PACKAGE_NAME}] Feature is not supported by browser! Disabling lazy processing of image nodes. [IntersectionObserver]`,
  ...
  if (!options._storage && options.cache.persistent)
    console.error(
      `[${PACKAGE_NAME}] Feature is not supported by browser! Disabling persistent cache of SVG files. [localStorage]`,
    ) // eslint-disable-line no-console
  ...
  if (options._storage && options.cache.removeRevisions)
    Object.entries(localStorage)
      ...
      .filter(
        (item) =>
          item.startsWith(`${PACKAGE_NAME}:`) && !item.endsWith(`:${options.cache.version}`),
      )
      .forEach((item) => localStorage.removeItem(item))

  /* create empty cache map or restore stored cache map */
  const cache =
    options._storage && options.cache.persistent
      ? new Map(JSON.parse(localStorage.getItem(CACHE_ID) || '[]'))
      : new Map()
  ...
  const createImageNodeIntersectionObserver = () => {
    ...
    if (!options._observer)
      throw new Error(
        `[${PACKAGE_NAME}] Feature is not supported by browser! [IntersectionObserver]`,
      )

    /* throw error if image node intersection observer already exists */
    if (refs.has(OBSERVER_REF_ID))
      throw new Error(
        `[${PACKAGE_NAME}] Can not create image node intersection observer, intersection observer already exists!`,
      )
  ...
  const getImageNodeIntersectionObserver = () => {
    /* return image node intersection observer reference */
    return refs.has(OBSERVER_REF_ID)
      ? refs.get(OBSERVER_REF_ID)
      : createImageNodeIntersectionObserver()
  }
  ...
  const createSvgSymbolContainer = () => {
    ...
    if (refs.has(CONTAINER_REF_ID))
      throw new Error(
        `[${PACKAGE_NAME}] Can not create SVG symbol container node, container node already exists!`,
      )

    /* create svg symbol container node */
    let container = createNode(
      `<svg xmlns="http://www.w3.org/2000/svg" id="${CONTAINER_ID}" style="display: none !important;"></svg>`,
    )
  ...
  const createAttributeMapFromNamedNodeMap = (namedNodeAttributeMap = null) => {
    ...
    const attributes = new Map(
      [...namedNodeAttributeMap].map(({ name, value }) => {
        ...
        if (!REGEXP_ATTRIBUTE_NAME.test(name))
          throw new TypeError(
            `[${PACKAGE_NAME}] Attribute name is not valid! [attribute="${name}"]`,
          )
      ...
      ;(options._axios ? options.axios.get : fetch)(file.path)
        ...
        .then((response) => {
          ...
          if (!CORRECT_RESPONSE_STATUSES.has(response.status | 0))
            throw new Error(`Wrong response status! [response.status=${response.status}]`) // PACKAGE_NAME prefix is not required here, it will be added in reject handler.
  ...
  const parseSvgFile = (file = null, node = null) => {
    ...
    if (!REGEXP_SVG_FILENAME.test(file.path))
      throw new TypeError(
        `[${PACKAGE_NAME}] Argument property is not valid! [file.path="${file.path}"]`,
      )
    ...
    if (!REGEXP_SVG_CONTENT.test(file.content))
      throw new TypeError(
        `[${PACKAGE_NAME}] Argument property is not valid! [file.content="${file.content}"]`,
      )
    ...
    if (node[FLAGS_ID].has('sprite')) {
      ...
      file.content = file.content.replace(REGEXP_SVG_CONTENT, (svg, attributes, symbol) => {
        ...
        return `
            ...
            options.attributes.clone.size && (attributes = createAttributeMapFromString(attributes))
              ? ` ${[...options.attributes.clone]
                  .filter((attribute) => !!attribute && attributes.has(attribute))
    ...
    return file.content.replace(REGEXP_SVG_CONTENT, (svg, attributes, symbol) => {
      ...
      for (const attribute of options.attributes.merge) {
        ...
        const fileValues = fileAttributes.has(attribute)
          ? fileAttributes
              .get(attribute)
              .split(REGEXP_WHITESPACE)
              .filter((value) => !!value)
          : [] // svg
        const nodeValues = nodeAttributes.has(attribute)
          ? nodeAttributes
              .get(attribute)
              .split(REGEXP_WHITESPACE)
              .filter((value) => !!value)
        ...
        attributes.set(
          attribute,
          (uniqueAttributeValues.has(attribute) ? [...new Set(values)] : values).join(' ').trim(),
        )
      }
      ...
      for (const attribute of options.attributes.add) {
        ...
        if (attributes.has(attribute.name)) {
          ...
          if (!options.attributes.merge.has(attribute.name))
            throw new Error(
              `[${PACKAGE_NAME}] Can not add attribute, attribute already exists. [${attribute.name}]`,
            )

          /* extract attribute values */
          const oldValues = attributes
            .get(attribute.name)
            .split(REGEXP_WHITESPACE)
            .filter((value) => !!value)
        ...
        attributes.set(
          attribute.name,
          (uniqueAttributeValues.has(attribute.name) ? [...new Set(values)] : values)
            .join(' ')
            .trim(),
        )
      }
      ...
      for (const attribute of options.attributes.data) {
        ...
        if (attributes.has(dataAttribute)) {
          ...
          if (!options.attributes.merge.has(dataAttribute))
            throw new Error(
              `[${PACKAGE_NAME}] Can not transform attribute to data-attribute, data-attribute already exists. [${attribute}]`,
            )
  ...
  const beforeMount = (node = null, binding = null, vnode = null) => {
    ...
    if (node.tagName !== 'IMG')
      throw new Error(`[${PACKAGE_NAME}] Required argument is not valid! [node]`)
    ...
    if (directives.length > 1)
      throw new Error(
        `[${PACKAGE_NAME}] Node has more than 1 directive! [${isVue3 ? 'vnode.dirs' : 'vnode.data.directives'}]`,
      )

    /* set internal sprite flag to image node */
    if (!!directives[0].modifiers[options.directive.spriteModifierName])
      node[FLAGS_ID].add('sprite') // eslint-disable-line no-extra-boolean-cast
